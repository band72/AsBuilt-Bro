...
# Let's fix the duplicated methods issue.
# The easiest way is to find `public void ExportAllToSingleFile(string path, string format)`
# wait, my regex can just find both `ExportToFolder` and `ExportAllToSingleFile` and replace their bodies. 

# But since I already broke it, let's see how broken it is.
# I will just write a regex that matches from `public void ExportToFolder(` to the next `public void ExportAllToSingleFile` entirely, 
# and also matches from `public void ExportAllToSingleFile(` to the end of the class.

# Actually, I can just replace BOTH methods entirely with perfectly clean C# code!!
...
clean_methods = """
    public void ExportToFolder(string folderPath, string format)
    {
        ...
        void Write<T>(string title, string[] headers, ObservableCollection<T> items, Func<T, object?[]> formatter)
        {
            if (isExcel)
            {
        ...
        Write("FigureAssets", hFigure, FigureAssets, i => new object?[] { i.Id, i.Name, i.Layer, "Description Removed", i.ScriptContent });
...
start_str = "    public void ExportToFolder(string folderPath, string format)"
end_str = "        public string Name { get; set; }"

start_idx = text.find(start_str)
end_idx = text.find(end_str)

new_text = text[:start_idx] + clean_methods + "    " + text[end_idx:]
