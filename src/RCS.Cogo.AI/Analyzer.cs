...
public class AiAnalyzer
{
    private static readonly HashSet<string> ValidCommands = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "AD", "AP", "ARCARC", "DISP", "LIST", "START", "CLOSE", "OC", "FS", "FIG", "A", "B", "L", "C", "D", "COPY-PT", "COPYPT", "DELPT",
    ...
    private void CheckCommandArguments(string cmd, string[] args, int lineNum, string rawLine, List<AiAnalysisResult> results)
    {
        ...
        switch (cmd)
        {
            ...
            case "COPY-PT":
            case "COPYPT":
                if (args.Length < 3) AddError("COPY-PT requires OldPoint and NewPoint numbers.", "COPY-PT <old_pt> <new_pt> [desc]");
                else 
                {
                    if (!IsNum(args[1]) || !IsNum(args[2])) AddWarn("Point Numbers usually correspond to numbers.", "While strings are allowed, convention suggests numbers.");
                }
                break;
            case "PT":
            case "PNT":
